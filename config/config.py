...
# GPU加速配置
USE_GPU = True  # 是否启用GPU加速（需要NVIDIA显卡和CUDA）
GPU_DEVICE_ID = 0  # GPU设备ID（如果有多个GPU）
GPU_MEMORY_FRACTION = 0.8  # GPU内存使用上限（80%）
GPU_ALLOW_GROWTH = True  # 是否允许GPU内存动态增长
AUTO_FALLBACK_TO_CPU = True  # GPU不可用或内存不足时自动降级到CPU
